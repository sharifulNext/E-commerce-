              ...
              <div className="flex items-start gap-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-5 mt-2">
                    ...
                    <p className="px-2 sm:px-3 sm:py-1 border border-gray-200 bg-slate-50 rounded">{item.size}</p>
                  </div>
                </div>
              </div>
              <input
                onChange={(e)=>e.target.value === '' || e.target.value === '0' ? null : updateQuantity(item._id, item.size,Number(e.target.value))}
                className="border border-gray-200 max-w-[40px] sm:max-w-[80px] px-1 sm:px-2 py-1 rounded"
                type="number"
                min={1}
